<?php
/**
 * create_nutrition_notification.php
 * 
 * Creates a notification for the user when they successfully add/update a nutrition plan.
 * 
 * POST Parameters:
 *   - user_id: ID of the user who added/updated the nutrition plan
 *   - pet_id: ID of the pet
 *   - pet_name: Name of the pet
 *   - daily_cups: Daily cups amount (optional)
 * 
 * Returns:
...
try {
    ...
    $daily_cups = isset($_POST['daily_cups']) ? trim($_POST['daily_cups']) : '';
    
    // Validate required parameters
    if (!$user_id || !$pet_id || empty($pet_name)) {
        http_response_code(400);
        echo json_encode([
            'status' => 'error',
            'message' => 'Missing required parameters: user_id, pet_id, and pet_name are required.'
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    $notification_type = 'nutrition_plan_added';
    $title = "Nutrition Plan Updated Successfully! 🍽️";
    $description = "You've updated the nutrition plan for $pet_name";
    if ($daily_cups) {
        $description .= " - $daily_cups daily";
    }
    $description .= ".";
    ...
    try {
        $insertStmt->execute([
            $user_id,
            null, // lost_pet_id (not applicable for nutrition plan)
            $notification_type,
            $title,
            $description,
            $pet_name,
            ...
            null, // location
            null, // lost_date
            $user_name // owner_name (the user who added/updated the nutrition plan)
        ]);
