<?php
/**
 * create_medication_notification.php
 * 
 * Creates a notification for the user when they successfully add a medication.
 * 
 * POST Parameters:
 *   - user_id: ID of the user who added the medication
 *   - pet_id: ID of the pet
 *   - pet_name: Name of the pet
 *   - medication_name: Name of the medication
 *   - dosage_time: Dosage time (optional)
 *   - frequency: Frequency of medication (optional)
 * 
 * Returns:
...
try {
    ...
    $medication_name = isset($_POST['medication_name']) ? trim($_POST['medication_name']) : '';
    $dosage_time = isset($_POST['dosage_time']) ? trim($_POST['dosage_time']) : '';
    $frequency = isset($_POST['frequency']) ? trim($_POST['frequency']) : '';
    
    // Validate required parameters
    if (!$user_id || !$pet_id || empty($pet_name) || empty($medication_name)) {
        http_response_code(400);
        echo json_encode([
            'status' => 'error',
            'message' => 'Missing required parameters: user_id, pet_id, pet_name, and medication_name are required.'
        ], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    $description = "You've added $medication_name for $pet_name";
    if ($dosage_time && $frequency) {
        $description .= " - $dosage_time, $frequency";
    } elseif ($dosage_time) {
        $description .= " - $dosage_time";
    } elseif ($frequency) {
    ...
    try {
        $insertStmt->execute([
            $user_id,
            null, // lost_pet_id (not applicable for medication)
            $notification_type,
            $title,
            $description,
            $pet_name,
            ...
            null, // location
            null, // lost_date
            $user_name // owner_name (the user who added the medication)
        ]);
